...
NT=1000;n=4;m=1; p=1; T = 0.1; Ntr=500;N=30;M=20;H=400;    
...
Up=f_Hankel(UD,M,H,m);
Uf=f_Hankel(UD(:,M+1:end),N,H,m);
Yp=f_Hankel(YD,M,H,p);
Yf=f_Hankel(YD(:,M+1:end),N,H,p);
...
Y=zeros(p,NS+N+1); Y(:,1:M)=YD(:,1:M);
Yk=zeros(p*(N+1)); 
U=zeros(m,NS+N); U(:,1:M)=UD(:,1:M);
umin=-5;umax=5;
Uk0=randn(m*N,1);
Uk1=ones(m*N,1);
...
for k=M+1:NS
   Yrk=Yr(k+1:k+N,1); 
   Ypk=reshape(Y(:,k-M+1:k),[],1);
   Upk=reshape(U(:,k-M+1:k),[],1);
